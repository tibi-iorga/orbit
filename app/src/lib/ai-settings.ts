...
export async function getSystemPrompt(organizationId: string): Promise<string> {
  const row = await prisma.appSetting.findUnique({
    where: { organizationId_key: { organizationId, key: "autogroup_system_prompt" } },
  });
...
export async function getOpenAIApiKey(organizationId: string): Promise<string | null> {
  const row = await prisma.appSetting.findUnique({
    where: { organizationId_key: { organizationId, key: "openai_api_key" } },
  });
  return row?.value?.trim() || process.env.OPENAI_API_KEY || null;
}

/** Returns where the key is sourced from */
export async function getApiKeySource(organizationId: string): Promise<"db" | "env" | "none"> {
  const row = await prisma.appSetting.findUnique({
    where: { organizationId_key: { organizationId, key: "openai_api_key" } },
  });
